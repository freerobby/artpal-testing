...
class WPTestActions extends WPTestCase {
	...
	function test_action_performance() {
		$a = new MockAction();
		$tags = array(rand_str(), rand_str(), rand_str(), rand_str(), rand_str());
		
		// two actions per tag
		foreach ($tags as $tag) {
			add_action($tag, array(&$a, 'action'));
			add_action($tag, array(&$a, 'action2'));
		}
		...
		printf("Action Time: %0.8f (%0.8f each)\n", $delta, $delta/$count);
	}
