...
// Goodbye Search Widget, Steira has search in the header and we don't need you.
function steira_unregister_widgets() {
	unregister_widget('WP_Widget_Search');
}
add_action('widgets_init', 'steira_unregister_widgets');
...
class Steira_About_Text extends WP_Widget {
	...
	function Steira_About_Text() {
		$widget_ops = array('classname' => 'widget_steira_about_text', 'description' => __('Text or HTML about your site, in the Steira style', 'theme'));
...
class Steira_Categories extends WP_Widget {
	function Steira_Categories() {
		$widget_ops = array('classname' => 'widget_steira_categories', 'description' => __('A grid of categories in the Steira Style', 'theme'));
	...
	function widget($args, $instance) {
		...
		echo $before_widget;
		$title = empty($instance['title']) ? __('Post Categories', 'theme') : apply_filters('widget_title', $instance['title']);
...
<?php
	...
	function widget( $args, $instance ) {
		...
		extract($args, EXTR_SKIP);
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Comments', 'theme') : $instance['title']);
...
?>
		<?php echo $before_widget; ?>
			...
			echo  '<li class="recentcomments"><a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '"><span class="name">' . get_comment_author() . '</span><span class="didwhat">' . __('commented on', 'theme')  . '</span><span class="title">' . get_the_title($comment->comment_post_ID) . '</span></a></li>';
			endforeach; endif;?></ul>
...
class Steira_Recent_Posts extends WP_Widget {
	function Steira_Recent_Posts() {
		$widget_ops = array('classname' => 'widget_steira_recent_posts', 'description' => __('The most recent posts on your blog in the Steira style', 'theme') );
		$this->WP_Widget('steira_recent_posts', __('Steira Recent Posts', 'theme'), $widget_ops);
...
?>
		...
		<li><a href="<?php the_permalink() ?>" title="<?php echo esc_attr(get_the_title() ? get_the_title() : get_the_ID()); ?>"><span class="date"><?php the_time('d/m/y'); ?></span> <span class="title"><?php if ( get_the_title() ) the_title(); else the_ID(); ?></span></a></li>
		<?php endwhile; ?>
		</ul>
		<?php echo $after_widget; ?>
<?php
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<div id="comment-<?php comment_ID(); ?>">
			<h4>
				...
				<span class="said"><?php _e('said','theme'); ?></span> 
...
function steira_search_form() {
	...
	$form .= '<input id="searchsubmit" class="submit" type="submit" value="Go" />';
	$form .= '</p>';
	$form .= '</form>';
