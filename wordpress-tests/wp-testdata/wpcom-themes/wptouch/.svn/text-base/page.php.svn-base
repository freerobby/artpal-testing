	 ...
	 <div class="page">
		<div class="page-title-icon">		
			...
			if ( isset( $mypages[get_the_ID()]) ) {
				...
				if ( file_exists( wptouch_dir() . '/images/icon-pool/' . $icon_name ) ) {
					$image = get_stylesheet_directory_uri() . '/images/icon-pool/' . $icon_name;	
				} else {
					$image = compat_get_upload_url() . '/wptouch/custom-icons/' . $icon_name;
				}
				echo('<img class="pageicon" src="' . $image . '" alt="icon" />'); 
			} else {
				echo ('<img class="pageicon" src="' . get_stylesheet_directory_uri() . '/images/icon-pool/Default.png" alt="pageicon" />');
			}
