...
<li id="comment-<?php comment_ID() ?>" <?php comment_class(sandbox_comment_class(false)); ?>>
	...
	<ul class="comment-meta">
		...
		<?php printf(__('<li>Posted %1$s at %2$s</li> <li><a href="%3$s" title="Permalink to this comment">Permalink</a></li>', 'chaostheory'),
			...
			?> <li><? edit_comment_link(__('(Edit)', 'chaostheory'), ' ', ''); ?> <?php comment_reply_link(array_merge( $args, array('add_below' => 'div-comment', 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?></li>
	</ul>
...
<?php if ( comments_open() ) : ?>
	<div id="respond">
	<h3><?php comment_form_title( __('Post a Comment', 'chaostheory'), __('Post a Comment to %s', 'chaostheory') ); ?></h3>
	<div id="cancel-comment-reply"><small><?php cancel_comment_reply_link() ?></small></div>

<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
	<div id="mustlogin"><?php printf(__('You must be <a href="%s" title="Log in">logged in</a> to post a comment.', 'chaostheory'),
...
<?php if ( $user_ID ) : ?>
			...
			<div class="form-one">
				<div id="loggedin"><?php printf(__('Logged in as <a href="%1$s" title="Logged in as %2$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'chaostheory'),
...
<?php else : ?>
			...
			<div class="form-one">
				<div class="form-label"><label for="author"><?php _e('Name', 'chaostheory') ?></label> <?php if ($req) _e('<span class="req-field">*</span>', 'chaostheory') ?></div>
				<div class="form-input"><input id="author" name="author" type="text" value="<?php echo $comment_author ?>" size="30" maxlength="20" tabindex="3" /></div>

				<div class="form-label"><label for="email"><?php _e('Email', 'chaostheory') ?></label> <?php if ($req) _e('<span class="req-field">*</span>', 'chaostheory') ?></div>
				<div class="form-input"><input id="email" name="email" type="text" value="<?php echo $comment_author_email ?>" size="30" maxlength="50" tabindex="4" /></div>

				<div class="form-label"><label for="url"><?php _e('Website', 'chaostheory') ?></label></div>
...
<?php endif /* if ( $user_ID ) */ ?>
			...
			<div class="form-two">
				<div class="form-label"><label for="comment"><?php _e('Comment', 'chaostheory') ?></label></div>
				<div class="form-textarea"><textarea id="comment" name="comment" cols="45" rows="8" tabindex="6"></textarea></div>

				<div class="form-submit">
				<input id="submit" name="submit" type="submit" value="<?php _e('Post Comment &raquo;', 'chaostheory') ?>" tabindex="7" />
