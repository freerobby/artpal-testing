...
<?php if (have_posts()) : ?>
		...
		<h2 class="page-title"><?php _e('Search Results for:', 'chaostheory') ?> <?php echo wp_specialchars(stripslashes($_GET['s']), true); ?></h2>
...
<?php while (have_posts()) : the_post(); ?>
			...
			<div id="post-<?php the_ID() ?>" class="<?php sandbox_post_class() ?>">
				<div class="entry-meta">
					<h2 class="entry-title"><a href="<?php the_permalink() ?>" title="<?php printf(__('Permalink to %s', 'chaostheory'), wp_specialchars(get_the_title(), 1)) ?>" rel="bookmark"><?php the_title() ?></a></h2>
					<ul>
...
<?php else : ?>
			...
			<form id="searchform" method="get" action="<?php bloginfo('home') ?>">
				<div>
					...
					<input id="searchsubmit" name="searchsubmit" type="submit" value="<?php _e('Search &raquo;', 'chaostheory') ?>" tabindex="2" />
				</div>
			</form>
