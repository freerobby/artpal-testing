...
function arras_addmenu() {
	...
	add_submenu_page( 'themes.php', __('Arras Theme', 'arras'), __('Theme Options', 'arras'), 8, 'arras-options', 'arras_admin' );
	add_submenu_page( 'themes.php', __('Arras Theme', 'arras'), __('Theme Guide', 'arras'), 8, 'arras-guide', 'arras_guide' );
...
function arras_admin() {
	...
	if ( isset($_GET['page']) && $_GET['page'] == 'arras-options' ) {
		...
		if ( isset($_REQUEST['save']) ) {
			...
			$notices = '<div class="updated"><p>' . __('Your settings have been saved.', 'arras') . '</p></div>';
		}
